...
image::intro-to-gbif::fig3.png[align="center", width="640"]
...
There are three points of access to GBIF-mediated data: http://www.gbif.org[GBIF.org^], the https://www.gbif.org/developer/summary[Application Programming Interface (API) services^], and https://www.gbif.org/occurrence-snapshots[cloud-based datasets^].
...
Examples include https://www.gbif.org/tool/81747/rgbif[rgbif^], https://www.rdocumentation.org/packages/CoordinateCleaner/versions/2.0-18[CoordinateCleaner^], and https://www.gbif.org/tool/OlyoYyRbKCSCkMKIi4oIT/pygbif-gbif-python-client[pygbif^], as well as, specialized tools that allow for more specific use cases, such as http://geocat.kew.org/[GeoCat^] for Red List assessments.

With regards to cloud-based datasets, GBIF makes data available on the https://planetarycomputer.microsoft.com/dataset/gbif[Microsoft Planetary Computer (Azure)^], as an https://registry.opendata.aws/gbif/[Amazon AWS Open Dataset^] and on a public https://console.cloud.google.com/storage/browser/public-datasets-gbif[Google GCS bucket^] and https://console.cloud.google.com/marketplace/product/bigquery-public-data/gbif-occurrences[BigQuery table^]. When using cloud-based snapshots, we always recommend creating a https://www.gbif.org/citation-guidelines#derivedDatasets[Derived Dataset citation^] for the records that you use. When referring to the full dataset, please use the appropriate citations as listed on the https://www.gbif.org/occurrence-snapshots[cloud-based datasets^] page.
...
image::intro-to-gbif::downloadoptions.png[align="center"]
...
TIP: https://techdocs.gbif.org/en/data-use/download-formats[Occurrence download formats^] are fully documented in GBIF's technical documentation.
...
TIP: https://techdocs.gbif.org/en/data-use/occurrence-issues-and-flags[Data quality issues and flags^] are documented in GBIF's technical documentation.
...
These most frequently occur from data entry errors or missing data fields whose interpretation can be automated centrally by GBIF. These interpretations are classified as: 
...
CAUTION: Be aware that if you are filtering for data quality issues, you should reverse the filter to exclude those data that have been flagged with that issue. You can also see the verbatim data i.e. the non-interpreted data in a Darwin Core Archive if you would like to validate the interpretation process.
...
GBIF produces https://www.gbif.org/composition/3NZQV4dMHw154WEwu4TqDw/guides-and-documentation#_gbif-digital-guides[digital guides^] on a range of topics relating to biodiversity informatics and open biodiversity data to support the community of practice.

=== Technical documentation

GBIF's https://techdocs.gbif.org/[technical documentation^] offers guidance on accessing, retrieving, and citing biodiversity data, with insights into data quality, publishing best practices, and resources for tools and standards that support data sharing.

=== GBIF data blog

The GBIF https://data-blog.gbif.org/[data blog^] is a place for GBIF staff and guest bloggers to contribute:
...
- [x] rGBIF
- [x] pyGBIF
- [x] occurrence snapshots in the cloud
....
// Question 2
