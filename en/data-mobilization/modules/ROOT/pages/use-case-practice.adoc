...
The observers use one of two standard protocols—-Pollard walks where the site is a transect and point counts at stationary point sites—-to capture this information on paper sheets that they send to the national SBN office. SBN processes the log files and produces an annual report based on the sightings.
...
* Each log contain observations of species that were actually present and observed at the surveyed sites, but the surveyor would have looked for all species that are included in the survey design. For visits to sites where no observations were made at all, "SpeciesIncludedInSurvey" will have a status set to "Absent". Additionally, information about whether a visit is a null visit (i.e. true = no survey species observed) or not (i.e. false = one or more survey species were observed) is included (see "noObservations").
