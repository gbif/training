= Navigating this course

== Content

You can obtain the content of each module in a written format. You will find the transcript below each presentation. Click to expand the transcript.

image::project-planning::transcript.png[align=left,width=50%,height=50%]

== Left navigation

You will find the course navigation on the left side of the page. Some of the menu items expand/collapse.

image::project-planning::left-nav.png[align=left,width=80%,height=80%]

&nbsp;

== Right navigation

You will find individual page contents on the right side of the page.

image::project-planning::right-nav.png[align=left,width=80%,height=80%]

&nbsp;

== Bottom navigation

At the bottom of each page you will find forward and back buttons to move you through the content.

image::project-planning::course-bottom-nav.png[]

&nbsp;

== Action icons

The course is comprised of modules. Each modules contains a series of actions for you to complete. This may include reading assignments, videos to watch, notes to take, and exercises to complete. 

Look for the following icons indicating you need to take action:

[NOTE.documentation]
Reading assignments

[NOTE.presentation]
Watch a video

[NOTE.quiz]
Check your understanding

[NOTE.activity]
Complete an activity or exercise

[NOTE.forum]
Discuss with your peers
